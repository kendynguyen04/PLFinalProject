...
public class Guild {
    public static void main(String[] args) {
        ...
        List<Object> e1 = Arrays.asList(1, "BOLLS", "MICHAEL", "BOLLSMICH", "3-MAR-90", "", "PRESIDENT", 4500, 0, 50, 0);
        List<Object> e2 = Arrays.asList(10, "JACKIE", "WELLS", "JACKWEL", "27-FEB-91", "", "WAREHOUSE MANAGER", 1507, 0, 45, 2);
        List<Object> e3 = Arrays.asList(11, "HENDERSON", "CHRIS", "HENDERCS", "14-MAY-90", "", "SALES REPRESENTATIVE", 1400, 10, 31, 3);
        List<Object> e4 = Arrays.asList(12, "GILSON", "JOHN", "GILSONSJ", "18-JAN-92", "", "SALES REPRESENTATIVE", 1490, 12.5, 32, 3);
        ...
        emp.add(e6); emp.add(e7); emp.add(e8); emp.add(e9); emp.add(e10);
        emp.add(e11); emp.add(e12); emp.add(e13); emp.add(e14); emp.add(e15);
        emp.add(e16); emp.add(e17); emp.add(e18); emp.add(e19); emp.add(e20);
        emp.add(e21); emp.add(e22); emp.add(e23); emp.add(e24); emp.add(e25);

        Guild.listG(emp);

        // Example Operation
        System.out.println("select last_name, first_name, userid, start_date, comments, title, salary, commission, dept_id, manager_id from s_emp");
        emp.stream().forEach(e -> {System.out.println(e);});

        // First Operation
        // Lists all the store clerks
        System.out.println("select last_name, first_name, userid, start_date, comments, title, salary, commission, dept_id, manager_id from s_emp where title = 'STORE CLERK'");
        emp.stream().filter(e -> e.get(6) == "STOCK CLERK").forEach(e -> {System.out.println(e);});

        // Second Operation
        System.out.println("select last_name from s_emp");
        emp.stream().map(e -> e.get(1).toString()).forEach(e -> {System.out.println(e);});

        // Third Operation
        System.out.println("select last_name from s_emp order by last_name");
        emp.stream().map(e -> e.get(1).toString()).sorted().forEach(e -> {System.out.println(e);});

        // Fourth Operation
        System.out.println("Number of Sales Representatives: " + emp.stream().filter(e -> e.get(6) == "SALES REPRESENTATIVE").count());

        // Fifth Operation
        System.out.println("select first(last_name) from s_emp");
        System.out.println(emp.stream().findFirst());

    }

    public static void listG(ArrayList<List<Object>> emp){
        System.out.println("select last_name, first_name, userid, start_date, comments, title, salary, commission, dept_id, manager_id from s_emp");
        emp.stream().forEach(e -> {System.out.println(e);});
    }
